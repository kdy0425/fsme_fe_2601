...
import { symTxccHC as baseHC } from '@/utils/fsms/headCells'

const BCrumb = [
  { to: '/', title: 'Home' },
  { title: '시스템관리' },
  { title: '연계정보관리' },
  { to: '/sym/txcc', title: '택시 카드협약사 연계관리' },
]
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const endpoint = '/fsm/sym/txcc/tx/getAllTaxiCardCntc' + toQueryParameter(searchObj)
  ...
  const handleActionClick = useCallback(async (row: Row, id: string) => {
    ...
    const endpoint = '/fsm/sym/txcc/tx/excuteTaxiCardCntc'
    const body = {
      procNm: row.procNm
    }
  ...
  const excelDownload = async () => {
    ...
    const endpoint = '/fsm/sym/txcc/tx/getExcelTaxiCardCntc' + toQueryParameter(searchObj)
    await getExcelFile(
      endpoint,
  ...
  const handleReport = async (start: string, end: string) => {
    ...
    const endpoint = '/fsm/sym/txcc/tx/getPrintTaxiCardCntc' + toQueryParameter(searchObj)
  ...
  return (
    <PageContainer title="택시 카드협약사 연계관리" description="택시 카드협약사 연계관리">
        ...
        <Box className="sch-filter-box">
          <div className="filter-form">
            <div className="form-group">
              <CustomFormLabel className="input-label-display" htmlFor="ft-procNm">
                프로세스명
              </CustomFormLabel>
              <CustomTextField
                type="text"
