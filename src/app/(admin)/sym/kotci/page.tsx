...
const BCrumb = [
  ...
  { title: '시스템관리' },
  { title: '연계정보관리' },
  { to: '/sym/kotci', title: '한국교통안전공단 연계 송수신정보' },
]
...
const DataList = () => {
  const [params, setParams] = useState<listSearchObj>({
    page: 1,
    size: 10,
    excnBgngYmd: getDateRange('d', 30).startDate,
    excnEndYmd: getDateRange('d', 30).endDate,
    dlngCd: '',
    procNm: '',
    instCd: INST_CD,
  })
  ...
  const searchValidation = () => {
    ...
    if (params.excnBgngYmd && params.excnEndYmd) {
      const startDate = new Date(params.excnBgngYmd)
      const endDate = new Date(params.excnEndYmd)
      const diffDays = (endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)
      if (diffDays > 90) {
        alert('조회 기간은 3개월을 초과할 수 없습니다.\n다시 확인해주세요.')
        return false
      }
    }
    return true
  ...
  const handleSearchChange = (
    ...
    if (name === 'instCd') {
      setParams((prev) => ({ ...prev, instCd: INST_CD }))
      return
    }
  ...
  const handleReport = async () => {
    if (params.excnBgngYmd !== params.excnEndYmd) {
      alert('출력은 실행일자가 하루(시작일자=종료일자)일 때만 가능합니다.');
      return;
    }
    ...
    try {
      ...
      if (
        ...
        const updatedData = response.data.map((item: any) => ({
          ...item,
          excnEndYmd: searchObj.excnEndYmd,
        }));
