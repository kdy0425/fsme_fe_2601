...
const RegisterModalForm = (props: ModalFormProps) => {
  ...
  const validation = (): boolean => {
    if (!params.cdGroupNm.trim()) {
      ...
      const msg = dataType === 'group' ? '코드그룹한글명을 입력해 주세요.' : '코드한글명을 입력해 주세요.'
      alert(msg)
  ...
  const handleSaveCode = async (isDel?: boolean) => {
    ...
    if (isDel) {
      userConfirm = confirm(dataType == 'group' ? '코드그룹정보를 삭제하시겠습니까?' : '공통코드를 삭제하시겠습니까?')
    } else if (formType === 'create') {
      userConfirm = confirm(dataType === 'group' ? '코드그룹정보를 등록하시겠습니까?' : '공통코드를 등록하시겠습니까?')
    } else {
      userConfirm = confirm(dataType == 'group' ? '코드그룹정보를 수정하시겠습니까?' : '공통코드를 수정하시겠습니까?')
    }
    ...
    if (userConfirm) {
      ...
      try {
        ...
        if (isDel) {
          endpoint = dataType === 'group' ? '/fsm/sym/cc/cm/deleteCmmnCdGroup' : '/fsm/sym/cc/cm/deleteCmmnCd'
          method = 'DELETE'
        } else if (formType === 'create') {
          endpoint = dataType === 'group' ? '/fsm/sym/cc/cm/createCmmnCdGroup' : '/fsm/sym/cc/cm/createCmmnCd'
          method = 'POST'
        } else {
          endpoint = dataType === 'group' ? '/fsm/sym/cc/cm/updateCmmnCdGroup' : '/fsm/sym/cc/cm/updateCmmnCd'
          method = 'PUT'
        }

        const response = await sendHttpRequest(method, endpoint, params, true, { cache: 'no-store' })
