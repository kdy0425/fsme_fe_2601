...
const RegisterModalForm = (props: ModalFormProps) => {
  ...
  const fetchParentCodes = async (): Promise<void> => {
    try {
      const searchObj = { clsfSeCd: 'B', useYn: 'Y' }
      const endpoint =
  ...
  const handleParamChange = (
    ...
    if (name === 'clsfCd') {
      if (value.length > MAX_CLSF_CD_LEN) {
        alert(`항목코드는 최대 ${MAX_CLSF_CD_LEN}자리까지 입력 가능합니다.`)
        return
      }
      if (value && !isAlphaNum(value)) {
        alert('항목코드는 영문 및 숫자 조합으로만 가능합니다.')
        return
      }
    ...
    if (name === 'clsfCdNm') {
      if (value.length > MAX_CLSF_CD_NM_LEN) {
        alert(`항목코드명은 최대 ${MAX_CLSF_CD_NM_LEN}자리까지 입력 가능합니다.`)
        return
      }
      setParams((prev) => ({ ...prev, [name]: value }))
      return
        ...
        <DialogContent>
          <Box className="table-bottom-button-group">
            <CustomFormLabel className="input-label-display">
              <h2>항목코드 {formType === 'create' ? '등록' : '수정'}</h2>
            </CustomFormLabel>
          ...
          >
            ...
            >
              <TableBody>
                ...
                <TableRow>
                  ...
                  >
                    <label htmlFor="modal-clsfCd">
                      <span className="required-text">*</span>항목코드
                    </label>
                  </TableCell>
                  <TableCell
                    style={{
                ...
                <TableRow>
                  <TableCell className="td-head table-title-column">
                    <label htmlFor="modal-clsfCdNm">
                      <span className="required-text">*</span>항목코드명
                    </label>
                  </TableCell>
                  <TableCell
                    style={{
                ...
                {/* 부모코드 – CustomSelect 빼고 native select 사용 */}
                <TableRow>
                  <TableCell className="td-head table-title-column">
