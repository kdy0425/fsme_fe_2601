...
import { stnDqiBsHc, stnDqiHisBsHc } from '@/utils/fsms/headCells2'
import {
  CtpvSelect,
  LocgovSelect,
  ...
  return (
    <TableHead>
      ...
      <TableRow>
        ...
        <TableCell style={{ whiteSpace: 'nowrap' }}>버스자격번호</TableCell>
        <TableCell style={{ whiteSpace: 'nowrap' }}>자격보유여부</TableCell>
        <TableCell style={{ whiteSpace: 'nowrap' }}>버스자격취득일자</TableCell>
        <TableCell style={{ whiteSpace: 'nowrap' }}>버스자격취소일자</TableCell>
...
export interface Row {
  ...
  busQlfcSttsNm?: string // 자격보유여부
  busQlfcSttsCdNm?: string // 자격보유여부
  busQlfcAcqsYmd?: string // 버스자격취득일자
  busQlfcRtrcnYmd?: string // 버스자격취소일자
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/dqi/bs/getAllDdpsQualfInfo?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/dqi/bs/getExcelDdpsQualfInfo?` +
        `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}` +
        `${params.vonrRrno ? '&vonrRrno=' + params.vonrRrno : ''}`
  ...
  const fetchhistoryData = async () => {
    ...
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string =
        `/fsm/stn/dqi/bs/getAllDdpsQualfHstInfo?` +
  ...
  const excelHisDownload = async () => {
    ...
    let endpoint: string =
      `/fsm/stn/dqi/bs/getExcelDdpsQualfHstInfo?` +
      `${'&vonrRrno=' + selectedRow.vonrRrno}`
    try {
      setLoadingBackdrop(true)
      await getExcelFile(
        endpoint,
        '버스운수종사자격정보_이력' + getToday() + '.xlsx',
      )
    } catch (error) {
      console.error('ERROR :: ', error)
    } finally {
