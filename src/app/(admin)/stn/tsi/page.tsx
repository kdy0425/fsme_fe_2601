...
import { stnTsiHC } from '@/utils/fsms/headCells'
...
export interface TaxiSalesInfoRow {
  vhclNo: string
  aplcnYmd: string
  useNmtm: number
  aplySeNm: string
...
type listSearchObj = {
  ...
  vhclNo: string
  aplySeCd: string
  srchDtGb: 'd' // 검색조건 분기처리
}

const DataList = () => {

  const [flag, setFlag] = useState<boolean | null>(null) // 데이터 갱신을 위한 플래그 설정
  const [rows, setRows] = useState<TaxiSalesInfoRow[]>([]) // 가져온 로우 데이터
  ...
  const fetchData = async () => {
    ...
    if (searchValidation()) {
      ...
      try {
        ...
        const endpoint = '/fsm/stn/tsi/cm/getAllTaxiSalesInfo' + toQueryParameter(searchObj)
  ...
  const excelDownload = async (): Promise<void> => {
    if (rows.length === 0) {
      ...
      const endpoint = `/fsm/stn/tsi/cm/getExcelTaxiSalesInfo` + toQueryParameter(searchObj)
  ...
  return (
    <PageContainer title="택시영업정보" description="택시영업정보">
      {/* breadcrumb */}
      <Breadcrumb title="택시영업정보" items={BCrumb} />
      ...
      <Box component="form" onSubmit={handleAdvancedSearch} sx={{ mb: 2 }}>
        ...
        <Box className="sch-filter-box">
          <div className="filter-form">
            ...
            <div className="form-group">
              <CustomFormLabel htmlFor="sch-aplySeCd" className="input-label-display">
                영업구분
              </CustomFormLabel>
              <CommSelect
                cdGroupNm="CCAC"
