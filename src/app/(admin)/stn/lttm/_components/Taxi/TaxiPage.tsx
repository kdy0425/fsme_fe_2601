...
const TaxiPage = () => {
  ...
  const cancleValidation = (item: Row): boolean => {
    return item.prcsSttsCd == '00' && item.dmndSeCd == '1' && item.chgLocgovCd === authLocgovCd
  }

  // 승인, 거절 가능건
  const confirmValidation = (item: Row): boolean => {
    return item.prcsSttsCd == '00' && item.dmndSeCd == '2' && item.chgLocgovCd === authLocgovCd
  }
  ...
  const handleSelectInterceptor = useCallback((pRow: Row): boolean => {
    if (cancleValidation(pRow) || confirmValidation(pRow)) {
      ...
      const msg = '처리상태 [이관요청], 요청구분명 [전출요청], 전입관청 [본인 지자체]\n처리상태 [이관요청], 요청구분명 [전입요청], 전입관청 [본인 지자체]\n로 등록된 데이터만 선택 가능합니다.'
      alert(msg)
      return false
    }
  }, [authLocgovCd])
          ...
          {selectedRow && (
            <BlankCard className="contents-card" title="상세 정보"
              buttons={[
                ...
                {
                  label: '취소',
                  disabled: !(selectedRow.prcsSttsCd == '00' && selectedRow.dmndSeCd == '1' && selectedRow.chgLocgovCd === authLocgovCd),
