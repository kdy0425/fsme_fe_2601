...
import { stnDdiTxHc } from '@/utils/fsms/headCells2'
import {
  CtpvSelect,
  LocgovSelect,
  CommSelect,
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/ddi/tx/getAllDrvngDstncInfo?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/ddi/tx/getExcelDrvngDstncInfo?` +
        `${params.bgngDt ? '&bgngDt=' + params.bgngDt.replaceAll('-', '') : ''}` +
        `${params.endDt ? '&endDt=' + params.endDt.replaceAll('-', '') : ''}` +
        `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
      ...
      await getExcelFile(endpoint, '택시운행거리정보_' + getToday() + '.xlsx')
    } catch (error) {
  ...
  return (
    <PageContainer title="택시 주행거리정보" description="택시 주행거리정보">
      {/* 검색영역 시작 */}
