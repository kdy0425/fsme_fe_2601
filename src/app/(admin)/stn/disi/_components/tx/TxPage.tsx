...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/disi/tx/getAllDutyInsrSbscInfo?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/disi/tx/getExcelDutyInsrSbscInfo?` +
        `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}`
      ...
      await getExcelFile(
        endpoint,
        '택시' + BCrumb[BCrumb.length - 1].title + getToday() + '.xlsx',
      )
    } catch (error) {
  ...
  const fetchhistoryData = async () => {
    ...
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string =
        `/fsm/stn/disi/tx/getAllDutyInsrSbscHst?page=${detailParams.page}&size=${detailParams.size}` +
        `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
  ...
  const excelHisDownload = async () => {
    ...
    let endpoint: string =
      `/fsm/stn/disi/tx/getExcelDutyInsrSbscHst?` +
      `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
      `${selectedRow.locgovCd ? '&locgovCd=' + selectedRow.locgovCd : ''}` +
      `${selectedRow.vhclNo ? '&vhclNo=' + selectedRow.vhclNo : ''}`
    try {
