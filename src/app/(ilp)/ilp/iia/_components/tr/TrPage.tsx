...
// 하이라이트된 행에만 __ins_* 플래그를 주입하고, 나머진 false로 초기화
const withInsFlagsOnlyHighlighted = (
  rows: Row[],
  highlightIdx: number,
  d8?: string | null
) => {
  const D8 = d8 ? toYmd8(d8) : null
  return rows.map((r, i) => {
    if (!(Number.isInteger(highlightIdx) && highlightIdx >= 0) || !D8) {
      // 하이라이트가 없거나 날짜가 없으면 전부 끈 상태
      return { ...r, __ins_reg: false, __ins_t1: false, __ins_t2: false, __ins_s: false }
    }
    if (i !== highlightIdx) {
      // 하이라이트 외 행 → 전부 끔
      return { ...r, __ins_reg: false, __ins_t1: false, __ins_t2: false, __ins_s: false }
    ...
    // 하이라이트된 행만 플래그 계산
    return markInsuranceFlagsForRow(D8, r, INS_MODE.insurance) as Row
  })
}
...
function DataList() {
  ...
  const fetchMain = useCallback(async (qp: ListSearch) => {
    ...
    try {
      const endpoint =
          `/ilp/iia/tr/dutyInsrSbscInfo?page=${qp.page}&size=${qp.size}` +
  ...
  const fetchHistory = useCallback(async (page: number, size: number, row?: Row, qp?: ListSearch|null) => {
    ...
    try {
      const endpoint =
          `/ilp/iia/tr/dutyInsrSbscHst?page=${page}&size=${size}` +
          `${qp.ctpvCd ? `&ctpvCd=${qp.ctpvCd}` : ''}` +
  ...
  const mainRowsDecorated = useMemo(() => {
    const d8 = toYmd8(analysisDate)
    // 날짜모드 && 하이라이트 존재일 때만 플래그 주입
    const base = (analysisMode === 'date' && d8 && mainHighlightIndex >= 0)
      ? withInsFlagsOnlyHighlighted(rows, mainHighlightIndex, d8)
      : rows.map(r => ({ ...r, __ins_reg: false, __ins_t1: false, __ins_t2: false, __ins_s: false }))

    // 기존 파란 하이라이트/체크 마킹은 유지 (__mark_* 사용)
    return decorateRows(base, mainHighlightIndex, mainFlags)
  }, [rows, analysisMode, analysisDate, mainHighlightIndex, mainFlags])

  // 히스토리 행: 날짜 모드일 때 각 행에 __ins_* 세팅(그리드가 컬럼별로 붉게 표시)
  const historyRowsDecorated = useMemo(() => {
    const d8 = toYmd8(analysisDate)

    const base = (analysisMode === 'date' && d8 && historyHighlightIndex >= 0)
      ? withInsFlagsOnlyHighlighted(historyRows, historyHighlightIndex, d8)
      : historyRows.map(r => ({ ...r, __ins_reg: false, __ins_t1: false, __ins_t2: false, __ins_s: false }))

    return decorateRows(base as any, historyHighlightIndex)
