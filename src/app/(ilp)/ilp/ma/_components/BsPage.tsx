...
import { maBsHc, maBsKoiHc, maBsLcnsHc } from '@/utils/fsms/ilp/headCells'
...
const DataList = () => {
 ...
 const handleMilegRowClick = (row: any) => {
  ...
  // srchDtGb는 "둘 다 true → BOTH, 하나만 true → KOI/LCNS, 둘 다 false → 이전 값 유지"
  useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (params.lcnsTpbizCd) qs.set('lcnsTpbizCd', String(params.lcnsTpbizCd))
      if (params.srchDtGb) qs.set('srchDtGb', String(params.srchDtGb))

      const endpoint = `/ilp/ma/bs/getMilegAnlsBs?${qs.toString()}`
  ...
  const excelDownload = async () => {
    ...
    try {
      ...
      const endpoint = `/ilp/ma/bs/getExcelMilegAnlsBs?${qs.toString()}`
      await getExcelFile(endpoint, `버스_연비분석_${getToday()}.xlsx`)
    } finally {
...
const headCells = useMemo(() => {
   switch (appliedGb) {
     case 'KOI':  return maBsKoiHc;
     case 'LCNS': return maBsLcnsHc;
     default:     return maBsHc; // BOTH
   }
 }, [appliedGb]);
...
useEffect(() => {
  ...
  params.koiCd,
  params.lcnsTpbizCd,
  params.srchDtGb,   // 유종별/면허업종별 체크로 바뀌는 값
]);
  ...
  return (
    <PageContainer title="차량관리" description="차량관리 페이지">
        ...
        <Box className="sch-filter-box">
          <div className="filter-form">
            ...
            <div className="form-group">
              ...
              <CommSelect
                cdGroupNm="505"
                pValue={params.lcnsTpbizCd as any}
                handleChange={handleSearchChange}
                pName="lcnsTpbizCd"
                htmlFor={'sch-lcns'}
                addText="전체"
      ...
      <MilegAnlsModal
        ...
        RowClickClose
        baseParams={params}
        baseGb={appliedGb}      // 'KOI' | 'LCNS' | 'BOTH'
        baseRows={rows}
      />
    </PageContainer>
  )
