...
import { vdHc } from '@/utils/fsms/ilp/headCells'

import VndcDmndModal from './_components/VndcDmndModal'
...
const DataList = () => {
  ...
  const searchParams = useSearchParams()   // 쿼리 파라미터 훅
  const [initAutoSearch, setInitAutoSearch] = useState(false)
  ...
  const [vndcMode, setVndcMode] = useState<'UPDATE'|'DELETE'>('UPDATE')
  ...
  useEffect(() => {
    ...
    // 2) 메인에서 ?prcsSttsCd=01 같이 넘어온 경우만 처리
    const qPrcsSttsCd = searchParams.get('prcsSttsCd') // 예: '01'
    
    if (qPrcsSttsCd) {
      // 처리상태를 쿼리값으로 세팅 + 첫 페이지로
      setParams(prev => ({
        ...prev,
        prcsSttsCd: qPrcsSttsCd ?? prev.prcsSttsCd,
        page: 1,
      }))

       //ctpvCd가 세팅되면 자동조회 하도록 플래그 설정
      setInitAutoSearch(true)
    }
  }, [])

  useEffect(() => {
    // 1) 메인에서 prcsSttsCd로 들어온 경우여야 하고
    // 2) 시도명(ctpvCd)이 실제로 선택된 상태여야 함
      if (initAutoSearch && params.ctpvCd) {
        setFlag(prev => !prev)      // 실제 fetchData() 트리거
        setInitAutoSearch(false)    // 한 번만 실행되도록 플래그 초기화
      }
  }, [initAutoSearch, params.ctpvCd])
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const endpoint = '/ilp/vd/cm/getVndcReqList' + toQueryParameter(searchObj)
