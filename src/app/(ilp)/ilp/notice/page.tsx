...
import { ilpNoticeHc } from '@/utils/fsms/headCells2'
...
const DataList = () => {
  ...
  const [params, setParams] = useState<listSearchObj>({
    page: 1,
    size: 10,
    startRegDt: getDateRange('d', 30).startDate,
  ...
  const searchValidation = () => {
    ...
    if (params.startRegDt > params.endRegDt) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const searchObj: listSearchObj = {
        ...params,
        startRegDt:  params.startRegDt.replaceAll('-', ''),
        endRegDt:  params.endRegDt.replaceAll('-', ''),
        srchDtGb: params.srchDtVal.trim() ? params.srchDtGb : '',
      }

      const endpoint = '/ilp/notice/getAllBoardDtl' + toQueryParameter(searchObj)
  ...
  const handleRowClick = useCallback((row: Row): void => {
    dispatch(handleIlpBoardOpen({ boardType: 'NOTICE', openType: 'VIEW', bbscttSn: row.bbscttSn }))
  }, [])

  // 등록
  const handleOpen = (): void => {
    dispatch(handleIlpBoardOpen({ boardType: 'NOTICE', openType: 'CREATE', bbscttSn: '' }))
  }
  ...
  return (
    <PageContainer title="공지사항" description="공지사항">
      ...
      <Box>
        <TableDataGrid
          headCells={ilpNoticeHc} // 테이블 헤더 값
          rows={rows} // 목록 데이터
       ...
       <>
          {ilpBoardOpen && pathname === '/ilp/notice' && (
