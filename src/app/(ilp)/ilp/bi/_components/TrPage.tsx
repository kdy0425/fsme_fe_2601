...
import { biStnBmBsnesMngTrHc } from '@/utils/fsms/ilp/headCells'
...
const TrPage = () => {
  ...
  const fetchData = async () => { 
    ...
    try {
      ...
      let endpoint: string =
        `/ilp/bi/tr/getAllBsnesMng?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
	...
	try{
	      ...
	      let endpoint: string =
	      `/ilp/bi/tr/getExcelBsnesMng?page=${params.page}&size=${params.size}` +
	          `${params.crno ? '&crno=' + params.crno : ''}`+
	          `${params.vonrRrno ? '&vonrRrno=' + params.vonrRrno : ''}` +
	          `${params.chk ? '&chk=' + params.chk : ''}` +
	          `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}` 

	      await getExcelFile(endpoint, '사업자정보_화물_'+getToday()+'.xlsx')
	    }catch(error){
  ...
  return (
    <Box>
      ...
      <Box>
        <TableDataGrid
          headCells={biStnBmBsnesMngTrHc} // 테이블 헤더 값
          rows={rows} // 목록 데이터
