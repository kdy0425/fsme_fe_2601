...
const BsPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      let endpoint: string =
        `/ilp/bi/bs/getAllBsnesMng?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/ilp/bi/bs/getExcelBsnesMng?page=${params.page}&size=${params.size}` +
        `${params.sort ? '&sort=' + sortChange(params.sort) : ''}` +
        `${params.brno ? '&brno=' + params.brno : ''}` +
        `${params.bzentyNm ? '&bzentyNm=' + params.bzentyNm : ''}` +
        `${params.rprsvNm ? '&rprsvNm=' + params.rprsvNm : ''}` +
  ...
  await  getExcelFile(endpoint, '사업자정보_버스_' + getToday() + '.xlsx')
    } catch (error) {
