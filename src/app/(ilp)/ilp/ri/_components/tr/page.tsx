...
import { riRfidTagJudgTrHeadCells } from '@/utils/fsms/ilp/headCells'
import {
  getDateRange,
  getExcelFile,
  getToday,
...
const TruckPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      if (schValidation()) {
        ...
        let endpoint: string =
          `/ilp/ri/tr/getAllRfidTagJdgmnMng?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    let endpoint: string =
      `/ilp/ri/tr/getExcelRfidTagJdgmnMng?` +
      `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}` +
      `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
      `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
      `${params.prcsSttsCd ? '&prcsSttsCd=' + params.prcsSttsCd : ''}` +
      ...
      await  getExcelFile(
      endpoint, 'RFID정보_화물_' + getToday() + '.xlsx')
  }
  ...
  return (
    <>
      ...
      <Box>
        <TableDataGrid
          headCells={riRfidTagJudgTrHeadCells} // 테이블 헤더 값
          rows={rows} // 목록 데이터
