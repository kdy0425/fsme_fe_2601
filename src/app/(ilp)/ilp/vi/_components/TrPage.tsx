...
import { ilpVmVhcleMngTrHc } from '@/utils/fsms/ilp/headCells'
...
const BasicTable = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/ilp/vi/tr/getAllVhcleMng?page=${params.page}&size=${params.size}` +
  ...
  return (
    <PageContainer title="차량관리" description="차량관리 페이지">
      ...
      <>
        {selectedRow && selectedRowIndex > -1 && (
          <Grid item xs={5} sm={4} md={4} style={{ marginBottom: '20px' }}>
            <BrnoDetailDataGrid data={selectedRow} />
          </Grid>
        )}
      </>
      ...
      <>
        {selectedRow && selectedRowIndex > -1 && (
          <Grid item xs={4} sm={4} md={4} style={{ marginBottom: '20px' }}>
            <CarDetailDataGrid data={selectedRow} />
          </Grid>
        )}
      </>
      ...
      <>
        {selectedRowIndex > -1 && (
          <Grid item xs={4} sm={4} md={4}>
            <MovePathReqDataGrid
              data={{ ...rows[selectedRowIndex], taskSeCd: 'TR' }}
            />
          </Grid>
        )}
      </>
