...
import { cmDutyInsrSbscInfoHeadCells, cmStnbiheadCells, cmVqiDrliHeadCells, bsVqiQualfInfoHeadCells  } from '@/utils/fsms/ilp/headCells'
...
const BasicTable = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/ilp/vqi/bs/getAllVhcleMng?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/ilp/vqi/bs/getExcelVhcleMng?page=${params.page}&size=${params.size}` +
        `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}` +
        `${params.bzentyNm ? '&bzentyNm=' + params.bzentyNm : ''}` +
      ...
      await getExcelFile(endpoint, '버스_차량자격_' + getToday() + '.xlsx')
    } catch (error) {
  ...
  const insrncParams = useMemo(() => {
    ...
    return {
      endpoint: '/ilp/vqi/bs/getAllDutyInsrSbscInfo', // 기본이면 생략 가능
  ...
  const qualfParams = useMemo(() => {
    if (!selectedRow) return undefined;
    return {
      endpoint: `/ilp/vqi/bs/getAllDdpsQualfInfo`, // 기본이면 생략 가능
      vonrRrno: selectedRow.vonrRrno, //주민번호확인
      vhclNo: selectedRow.vhclNo,
      //ctpvCd: (params.ctpvCd as string) || undefined,
      //locgovCd: (params.locgovCd as string) || undefined,
      page: 1,
  ...
  }, [selectedRow?.rprsvRrno, params.ctpvCd, params.locgovCd]);
    ...
    <PageContainer title="차량관리" description="차량관리 페이지">
      ...
      <>
        {selectedRow && selectedRowIndex > -1 &&(
          ...
          > 
            <TableDataGrid
              headCells={bsVqiQualfInfoHeadCells}
              rows={qualfRows}
