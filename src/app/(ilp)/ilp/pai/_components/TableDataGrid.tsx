            ...
            {loading ? (
              ...
              rows.map((row, index) => (
                ...
                >
                  {headCells.map((headCell) => (
                    <TableCell key={headCell.id} align={headCell.numeric ? 'right' : 'left'}>
                      {headCell.id === 'excelDownloadYn' 
                        ? (row[headCell.id] === 'Y' ? '다운로드' : '미다운로드')
                        : headCell.id === 'searchDetailDt' && row[headCell.id]
                        ? new Date(row[headCell.id]).toLocaleString('ko-KR')
                        : headCell.id === 'searchDt' && row[headCell.id]
                        ? row[headCell.id].replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3')
                        : row[headCell.id] || '-'
                      }
                    </TableCell>
                  ))}
