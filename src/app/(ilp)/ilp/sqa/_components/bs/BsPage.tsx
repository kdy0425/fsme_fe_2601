...
import { sqaBsHc, sqaInsrncCmHC, sqaTransprtBsHc } from '@/utils/fsms/ilp/headCells'
...
interface Row {
  ...
  sttsCd?: string
  busQlfcNo?: string
  busQlfcSttsNm?: string
  busQlfcAcqsYmd?: string
  busQlfcRtrcnYmd?: string | null
  kotsaRgtrId?: string
  kotsaRegDt?: string
  kotsaMdfrId?: string
  kotsaMdfcnDt?: string
...
const calcInvalidDaysForPeriodStrict = (r: Row, from8: string, to8: string) => {
  ...
  const badBus = (r.busQlfcSttsNm ?? '').trim() !== '취득'
  if (badVhcl || badBus) return days
  const kReg = toYmd8(r.kotsaRegDt)
  const iReg = toYmd8(r.regDt)
...
const isbusValidOnDate = (d8: string, qRows: AnyRow[]) => {
  return (qRows ?? []).some(r => {
    if (isFH(r)) return false;
    const k8   = toYmd8LooseFromKeys(r, ['kotsaRegDt'])
    const acq8 = toYmd8LooseFromKeys(r, ['busQlfcAcqsYmd'])
    const rtr8 = toYmd8LooseFromKeys(r, ['busQlfcRtrcnYmd']) ?? '99991231'
    const acquired = String(r?.busQlfcSttsNm ?? '').trim() === '취득'
...
const calcInvalidDaysForPeriodDetailed = (
  ...
  return days.filter(ymd => {
    ...
    const busOK = isbusValidOnDate(d8, qRows)
    return !(insOK && busOK) // 둘 다 충족해야 유효
  })
}
...
const callAnalyzeBs = async (params: {
  ...
  const endpoint = `/ilp/sqa/bs/getAllSpldmdQlfcAnlsBs?${qs}`
...
const normalizeQualfRow = (r: AnyRow, mainRow?: Row) => ({
  ...
  busQlfcNo:       r.busQlfcNo       ?? r.BUS_QLFC_NO,
  busQlfcSttsNm:   r.busQlfcSttsNm   ?? r.BUS_QLFC_STTS_NM ?? r.STATUS_NM,
  busQlfcAcqsYmd:  r.busQlfcAcqsYmd  ?? r.BUS_QLFC_ACQS_YMD ?? r.ACQ_DT,
  busQlfcRtrcnYmd: r.busQlfcRtrcnYmd ?? r.BUS_QLFC_RTRCN_YMD,
  ...
  sttsCd:        r.sttsCd        ?? r.BUS_QLFC_STTS_CD,
});
...
function isMainInvalidForDate(r: Row, d8: string) {
  ...
  const acq8 = toYmd8(r.busQlfcAcqsYmd) ?? '99991231'
  const rtr8 = toYmd8(r.busQlfcRtrcnYmd) ?? '99991231'
  const inBusRange = cmp(acq8, d8) <= 0 && cmp(d8, rtr8) <= 0
  const acquired = String(r.busQlfcSttsNm ?? '').trim() === '취득'

  const badBus = !(kOk && acquired && inBusRange)
  return badVhcl || badIns || badBus
}
...
function DataList() {
  ...
  const insrncParams = useMemo(() => {
    ...
    return {
      endpoint: '/ilp/sqa/bs/getAllDutyInsrSbscInfo',
  ...
  const qualfParams = useMemo(() => {
    if (!selectedRow) return undefined
    return {
      endpoint: '/ilp/sqa/bs/getAllDdpsQualfInfo',
  ...
  // useEffect(() => setInsrncViewRows(insrncRows ?? []), [insrncRows])
  // useEffect(()  => setQualfViewRows(qualfRows  ?? []), [qualfRows])
  useEffect(() => {
  ...
  const fetchMain = useCallback(
    async (qp: ListSearch) => {
      clearDetailMarks()
      //const userInfoRef = getUserInfo()
      //const isLogv = userInfoRef?.roles?.[0] === 'LOGV'
      if (isLogv) {
      ...
      try {
        const endpoint =
          `/ilp/sqa/bs/getAllDdpsQualfInfo?page=${qp.page}&size=${qp.size}` +
  ...
  useEffect(() => {
        ...
        try {
            ...
            if (res) {
              ...
              const qRows = (res.qualf ?? []).map((raw, i) => {
                ...
                const acquired = String(r?.busQlfcSttsNm ?? '').trim() === '취득';
                return {
                  id: raw.id ?? r.busQlfcNo ?? r.hstrySn ?? i,
                  ...r,
                  __ql_fh:   isFH,
              ...
              const okQ = (res.qualf ?? []).some((r) => {
                ...
                const acq8 = toYmd8LooseFromKeys(r, ['busQlfcAcqsYmd','BUS_QLFC_ACQS_YMD','ACQ_DT']);
                const rtr8 = toYmd8LooseFromKeys(r, ['busQlfcRtrcnYmd','BUS_QLFC_RTRCN_YMD','RTR_DT']) ?? '99991231';
                const inRangeQ = !!acq8 && cmp(acq8, d8) <= 0 && cmp(d8, rtr8) <= 0;
                const acquired = String(r?.busQlfcSttsNm ?? r?.BUS_QLFC_STTS_NM ?? '').trim() === '취득';
            ...
            const newQlf = (qualfRows ?? []).map((r: AnyRow) => {
              ...
              const acq8 = toYmd8LooseFromKeys(r, ['busQlfcAcqsYmd'])  ?? '99991231';
              const rtr8 = toYmd8LooseFromKeys(r, ['busQlfcRtrcnYmd']) ?? '99991231';
              return { ...r,
    ...
    if (analysisMode === 'period') {
      ...
      // const picked = (() => {
      //   const fits = mains.filter(({ row }) => overlapEitherPeriod(row, f8, t8))
      ...
      //   // 상세행을 사용해 날짜별 유효성 판정(의무보험 + 운수종사자)
      //   const days = calcInvalidDaysForPeriod(
      //     picked.row,
      //     insrncRows ?? [],
      //     qualfRows ?? [],
      //     f8,
    ...
    const withFlags = rows.map((r, i) => {
      ...
      if (analysisD8) {
        ...
        if (i === selectedIndex) {
          ...
          const busOK = isbusValidOnDate(analysisD8, qlfSrc)
          base.busQlfcSttsNmAnls = busOK ? Blue('유효') : Red('미유효')
        } else {
          base.twdpsn1SeNmAnls = base.twdpsn2SeNmAnls = base.sbsttNmAnls = base.busQlfcSttsNmAnls = null
        }
      }
  ...
  const doExportExcel = useCallback(() => {
    // 1) 체크열 제외
    const exportHeads = (sqaBsHc as any).filter((h: any) => h.id !== 'check') // chk 제외
    ...
    exportGridToXlsx({
      ...
      filename: '수급자격_종합분석_버스.xlsx',      // 고정 파일명
      sheetName: '버스-수급자격',
