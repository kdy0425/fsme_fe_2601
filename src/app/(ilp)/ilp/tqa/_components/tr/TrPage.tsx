...
// ───────── 유효/비유효 판정(요구: frghtQlfcSttsNm 기반) ─────────
const isPsnInvalid = (r: Row) => (r.frghtQlfcSttsNm ?? '').trim() !== '취득'
...
function DataList() {
  ...
  const fetchMain = useCallback(
    async (qp: ListSearch) => {
      ...
      try {
        const endpoint =
          `/ilp/tqa/tr/ddpsQualfInfo?page=${qp.page}&size=${qp.size}` +
  ...
  const fetchHistory = useCallback(
    async (page: number, size: number, row?: Row, qp?: ListSearch | null, opts?: { keepHighlight?: boolean }) => {
      ...
      try {
        ...
        const endpoint =
          `/ilp/tqa/tr/ddpsQualfHst?page=${page}&size=${size}` +
          `${row.kotsaVonrRrno ? '&vonrRrno=' + row.kotsaVonrRrno : '&vonrRrno=' + row.driverVonrRrno }`
  ...
  useEffect(() => {
    ...
    const chooseFromHistoryAllPages = async (
      ...
      for (let p = 1; p <= totalPages; p++) {
        const endpoint =
          `/ilp/tqa/tr/ddpsQualfHst?page=${p}&size=${detailPageable.pageSize}` +
          `${selectedRow.kotsaVonrRrno ? '&vonrRrno=' + selectedRow.kotsaVonrRrno : '&vonrRrno=' + selectedRow.driverVonrRrno}`
