...
function convertRowToMenuitemsType(menu: Menu): MenuitemsType {
  return {
    id: menu.menuTsid,
    navlabel: menu.upMenuTsid === '1000000000000' ? true : false,
    subheader: menu.upMenuTsid === '1000000000000' ? menu.menuNm : undefined,
...
const SidebarItems = () => {
  ...
  const fetchData = async () => {
    try {
      const endpoint = `/fsm/cmm/cmmn/cm/getAllUserIlpMenu`
  ...
  const isParentActive = (menuItem: MenuitemsType): boolean => {
    if (menuItem.href && pathname?.split('?')[0] === menuItem.href) {
      return true
    }
